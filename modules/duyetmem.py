...
def handle_duyetmem_command(message, message_object, thread_id, thread_type, author_id, client):
    try:
        ...
        if author_id not in all_admin_ids and author_id not in ADMIN:
            client.replyMessage(
                Message(text="Mày không có quyền sử dụng lệnh này."),
        ...
        if len(command_parts) < 2:
            client.replyMessage(
                Message(text="Lệnh không hợp lệ. Vui lòng sử dụng `duyetmen list` để xem danh sách hoặc `duyetmen all` để duyệt tất cả."),
                message_object, thread_id, thread_type
            )
            return
...
def get_mitaizl():
    return {
        'duyetmen': handle_duyetmem_command
    }
