...
def stop_onetag(client, message_object, thread_id, thread_type):
    ...
    is_onetag_running = False
    client.replyMessage(Message(text="Tha Lũ Gay."), message_object, thread_id, thread_type)
...
def handle_onetag_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="lũ óc cặc xin cha NhatMinh chưa mà dùng vậy cn đĩ mẹ chmay ảo quyền nặng rồi à cn đĩ tâm thần👉🧠🫵"),
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: on hoặc stop)."), message_object, thread_id, thread_type)
        return
    ...
    if action == "stop":
        if not is_onetag_running:
            client.replyMessage(
                Message(text="⚠️ ** Cha Tạm Tha Lũ Gay.**"),
                message_object, thread_id, thread_type
            )
        else:
    ...
    if message_object.mentions:
        ...
        client.replyMessage(Message(text="Đợi Tí NhatMinh Vui Lòng Chọn Người Muốn Tag"), message_object, thread_id, thread_type)
        return

    try:
        with open("5ctag.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
