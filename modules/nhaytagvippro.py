...
def stop_onetag(client, message_object, thread_id, thread_type):
    ...
    is_onetag_running = False
    client.replyMessage(Message(text="Đã dừng Tag tên."), message_object, thread_id, thread_type)
...
def handle_onetag_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="lũ óc cặc xin cha 𝙃𝙖̀ 𝙃𝙪𝙮 𝙃𝙤𝙖̀𝙣𝙜 chưa mà dùng vậy cn đĩ mẹ chmay ảo quyền nặng rồi à cn đĩ tâm thần👉🧠🫵"),
