...
def handle_reo_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="𝙃𝙖̀ 𝙃𝙪𝙮 𝙃𝙤𝙖̀𝙣𝙜 Mới Sử Được🤪"),
    ...
    if action == "stop":
        if not is_reo_running:
            client.replyMessage(
                Message(text="⚠️ **Bố 𝙃𝙖̀ 𝙃𝙪𝙮 𝙃𝙤𝙖̀𝙣𝙜 Tạm Tha /-li.**"),
                message_object, thread_id, thread_type
            )
        else:
    ...
    if message_object.mentions:
        ...
        client.replyMessage(Message(text="Đợi Tí ! Vui Lòng Chọn Người Muốn Chửi"), message_object, thread_id, thread_type)
        return

    try:
        with open("rain.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    except FileNotFoundError:
        client.replyMessage(
            Message(text="Không tìm thấy file rain.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File rain.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
