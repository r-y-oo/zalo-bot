...
des = {
    ...
    'description': "Gửi nội dung từ file bua.txt liên tục trong nhóm."
}
...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        with open("bua.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File bua.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
