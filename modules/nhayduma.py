...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: dí on hoặc stop)."), message_object, thread_id, thread_type)
        return
    ...
    try:
        ...
        client.replyMessage(
            Message(text="Không tìm thấy file trollv2.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File trollv2.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
