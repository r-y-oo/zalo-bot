...
def handle_reo_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="lũ ngu chúng mày xin cha 𝙃𝙖̀ 𝙃𝙪𝙮 𝙃𝙤𝙖̀𝙣𝙜 chưa mà dùng vậy em ảo quyền à?"),
    ...
    try:
        ...
        client.replyMessage(
            Message(text="Không tìm thấy file choc.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File choc.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
