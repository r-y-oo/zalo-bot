...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="LŨ NGU CẶN BÃ NÀY M XIN CHA 𝙃𝙖̀ 𝙃𝙪𝙮 𝙃𝙤𝙖̀𝙣𝙜 CHƯA MÀ DÙNG VẬY CN ĐĨ MẸ MÀY ẢO QUYỀN CN CẶC CHA M TREO CỔ CẢ HỌ M CHẾT CHÁY 👉🧠🫵😂😂??."),
    ...
    try:
        with open("nhaychtme.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    except FileNotFoundError:
        client.replyMessage(
            Message(text="Không tìm thấy file nhaychtme.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File nhaychtme.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
