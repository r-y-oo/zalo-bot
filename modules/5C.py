...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="LŨ ÓC CẶC LGBT CHÚNG MÀY XIN CHA Ng Dag Thai CHƯA MÀ DÙNG ??."),
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: 5c on hoặc 5c stop)."), message_object, thread_id, thread_type)
        return
    ...
    if action == "stop":
        if not is_war_running:
            client.replyMessage(
                Message(text="⚠️ **Thưa Cha Ng Dag Thai Tạm Tha Lũ Đú.**"),
                message_object, thread_id, thread_type
            )
        else:
