...
def handle_sendmsg_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="bạn chưa được duyệt để sài bot ! vui lòng liên hệ admin ChanKhang để được duyệt sài bot 🤖"),
    ...
    try:
        with open("nhaytoichet.txt", "r", encoding="utf-8") as file:
...
def handle_stop_command(message, message_object, thread_id, thread_type, author_id, client):
    global stop_spam
    stop_spam = True
    client.sendMessage(Message(text="Đã dừng spam tin nhắn."), thread_id, thread_type,ttl=30000)

def get_mitaizl():
    return {
        'chuichtme': handle_sendmsg_command,
        'chuichtme_stop': handle_stop_command
    }