...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Cn đĩ lồn này m bú cặc cha riết ngu rồi ảo quyền à zin cha nhật minh chưa mà dùng thế cn đĩ ngâu👉🧠🫵 ??."),
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: nhay5c on hoặc stop)."), message_object, thread_id, thread_type)
        return
