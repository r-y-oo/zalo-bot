...
def handle_reo_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="ảo lồn à cn đĩ vô danh m đã xin cha DEXRY đâu mà m đòi"),
    ...
    if message_object.mentions:
        ...
        client.replyMessage(Message(text="Đợi Tí ! DEXRY Vui Lòng Chọn Người Muốn Réo"), message_object, thread_id, thread_type)
        return

    try:
        with open("reo.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
