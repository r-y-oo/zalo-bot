...
def handle_reo_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="PHẾ NHÂN CỦA MXH NHƯ MÀY KHÔNG CÓ QUYỀN🤪"),
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: toxictag on hoặc stop)."), message_object, thread_id, thread_type)
        return
    ...
    try:
        ...
        client.replyMessage(
            Message(text="Không tìm thấy file toxic.txt."),
            message_object,
            thread_id,
            thread_type
        )
