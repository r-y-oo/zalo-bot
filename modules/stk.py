    ...
    if message_object.quote:
        ...
        if attach:
            ...
            if webp_video_url:
                ...
                client.sendMessage(
                    Message(text="mỗi lần tạo 1 cái đợi cái khác làm xong r reply tiếp."),
                    thread_id=thread_id,
                    thread_type=thread_type
                )
        else:
        ...
        if message.startswith('#stk '):
            ...
            client.sendMessage(
                Message(text="bruh ko reply bt ảnh nào mà tạo,"),
                thread_id=thread_id,
                thread_type=thread_type
            )
...
def convert_video_to_webp(video_url):
    try:
        ...
        subprocess.run(['ffmpeg', '-y', '-i', 'temp.mp4', '-t', '15000', '-vcodec', 'libwebp', '-loop', '0', '-q:v', '60', 'output.webp'], check=True)
