...
def handle_onetag_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: nhayvip on hoặc stop)."), message_object, thread_id, thread_type)
        return
    ...
    if message_object.mentions:
        ...
        client.replyMessage(Message(text="Đợi Tí ! nhatmimh Vui Lòng Chọn Người Muốn Tag"), message_object, thread_id, thread_type)
        return

    try:
        with open("trollv2.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
