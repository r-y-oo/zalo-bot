...
des = {
    'version': "1.0.2",
    'credits': "TranGiaBao",
    'description': "Gửi nội dung từ file 4.txt liên tục trong nhóm."
}
...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Làm Lông Gì Vậy Em Xin Cha Ng Dag Thai Chưa Mà Sài."),
    ...
    if action == "stop":
        if not is_war_running:
            client.replyMessage(
                Message(text="⚠️ **Đã Tạm Tha Lũ Gay Thưa Cha Ng Dag Thai**"),
                message_object, thread_id, thread_type
            )
        else:
    ...
    try:
        ...
        client.replyMessage(
            Message(text="Không tìm thấy file 4.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File 4.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
