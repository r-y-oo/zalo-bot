...
def create_banner(client, member_id, member_name, total_member, group_name, avatar_url, ow_name, t=1):
    ...
    if t==1:
        ...
        additional_texts=[f"Đã tham gia nhóm {group_name}", "                                          convert by ©AntiMini"]#xoá cre làm choá :)))
    elif t ==2:
        main_text=f'Bye , Con Chó {user_name}💔'
        additional_texts=[f"Đã rời nhóm {group_name}", "                                          convert by ©AntiMini"]#có phải anh chiều em quá nên em hư đúng không 
    else:
        main_text=f' Thằng oắt con {user_name}💢'
        additional_texts=[f"đã bị {ow_name} sút khỏi nhóm {group_name} Vì Quá Ngu", "                                          convert by ©AntiMini"]#thôi được rồi -)))
    ...
    try:
        ...
        for member in event_data.updateMembers:
            ...
            if event_type == GroupEventType.JOIN:
                ...
                msg = f"🎉 Chào mừng {member_name} đã gia nhập {group_name.upper()} /-li"
                ...
                msg = Message(text=f"💔 Chào tạm biệt {member_name} /-li")
                try:
                ...
                msg = Message(text=f'/-li Chúc mừng {member_name} đã được {ow_name} bổ nhiệm làm quản trị viên của nhóm {group_name.upper()}')
                ...
                msg = Message(text=f'/-li Quản trị viên {ow_name} đã cập nhật mô tả của nhóm {group_name.upper()}')
                client.sendMessage(msg, thread_id, thread_type, mark_message='important', ttl=60000*69)
