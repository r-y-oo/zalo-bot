...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Phải là Cha Ng Dag Thai mới sử dụng được lệnh này nha con đĩ lồn."),
    ...
    if action == "stop":
        if not is_war_running:
            client.replyMessage(
                Message(text="⚠️ **Thưa Cha Ng Dag Thai Đã Tạm Tha Lũ Ngu**"),
                message_object, thread_id, thread_type
            )
        else:
