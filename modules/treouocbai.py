...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Thằng Mặt Cặc Lồn Thâm Được Phép Xử Dụng À Em Con Đĩ Mẹ Mày Nun Nha Con Chó Ngu Phèn Phế Chết Con Đĩ Mẹ Nun Mà Đụ Mẹ Đòi Dài Bot Của Cha À Em Djt Mẹ Mày Ảo Tưởng Ít Thôi Cẩm Phán Bố Đi Rồi Đụ Mẹ Bố Cho M Về Bú Sữa Mẹ Nha Cchos Dốt🤪😎👊 ??."),
    ...
    try:
        with open("uocrot.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    except FileNotFoundError:
        client.replyMessage(
            Message(text="Không tìm thấy file uocrot.txt."),
            message_object,
            thread_id,
            thread_type
        )
