...
class ZaloAPI(object):
    ...
    def changeGroupSetting(self, groupId, defaultMode="default", **kwargs):
       ...
       Args:
           ...
           **kwargs: Group settings kwargs, Value: (1 = True, 0 = False)
              ...
              blockName: KhĂ´ng cho phĂ©p user Ä‘á»•i tĂªn & áº£nh Ä‘áº¡i diá»‡n nhĂ³m
              signAdminMsg: ÄĂ¡nh dáº¥u tin nháº¯n tá»« chá»§/phĂ³ nhĂ³m
              addMemberOnly: Chá»‰ thĂªm members (Khi táº¯t link tham gia nhĂ³m)
              setTopicOnly: Cho phĂ©p members ghim (tin nháº¯n, ghi chĂº, bĂ¬nh chá»n)
              enableMsgHistory: Cho phĂ©p new members Ä‘á»c tin nháº¯n gáº§n nháº¥t
              lockCreatePost: KhĂ´ng cho phĂ©p members táº¡o ghi chĂº, nháº¯c háº¹n
              lockCreatePoll: KhĂ´ng cho phĂ©p members táº¡o bĂ¬nh chá»n
              joinAppr: Cháº¿ Ä‘á»™ phĂª duyá»‡t thĂ nh viĂªn
              bannFeature: Default (No description)
              dirtyMedia: Default (No description)
              banDuration: Default (No description)
              lockSendMsg: KhĂ´ng cho phĂ©p members gá»­i tin nháº¯n
              lockViewMember: KhĂ´ng cho phĂ©p members xem thĂ nh viĂªn nhĂ³m
              blocked_members: Danh sĂ¡ch members bá»‹ cháº·n
    ...
    def onLoggingIn(self, phone=None):
       ...
       """
       logger.debug("Logging in Duy X Khanh {}...".format(phone))
